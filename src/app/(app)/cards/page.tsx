      ...
      {cards.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {cards.map((card) => {
            ...
            return (
              <article key={card.id} className="rounded-lg border border-line bg-surface-1 p-5 shadow-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate font-display text-lg font-semibold text-text-hi">{card.bank}</p>
                    <p className="truncate text-sm text-text-lo">
                      {card.product} · vence dia {card.dueDay}
                    </p>
                  </div>
                ...
                <div className="mt-4 flex items-end justify-between">
                  ...
                  <p className="text-sm text-text-lo">
                    de <Money cents={card.limitCents} withSign={false} />
                  </p>
                </div>
